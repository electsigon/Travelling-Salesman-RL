...
#This code is largely inspired by https://github.com/ast0414/pointer-networks-pytorch, with modifications to make it work in an RL environment
...
def masked_sample(vector, mask, dim, keepdim=False, min_val = -1e7):
        one_minus_mask = (1.0 - mask).byte()
        replaced_vector = vector.masked_fill(one_minus_mask.bool(), min_val)
        prob_dist = torch.distributions.Categorical(logits = replaced_vector)
        index = prob_dist.sample()
        log_probs = prob_dist.log_prob(index)
        index = index.reshape((len(index), 1))
        return index, log_probs.reshape((len(index), 1))
...
class PointerNet(nn.Module):
	...
	def forward(self, input_seq, input_lengths):
		...
		for i in range(max_seq_len):
            ...
            #This makes it so that no element can be picked twice in a permutation
			one_hot = torch.zeros(sub_mask.size())
			if(len(pointer_argmaxs) > 0):
			        sub_mask -= one_hot.scatter_(1, torch.cat(pointer_argmaxs, 1), 1)
			...
			masked_argmax, selected_log_score = masked_sample(log_pointer_score, sub_mask, dim=1, keepdim=True)

			selected_log_scores.append(selected_log_score)
		...
		selected_log_scores = torch.cat(selected_log_scores, 1)
		return pointer_log_scores, selected_log_scores, pointer_argmaxs, mask_tensor
